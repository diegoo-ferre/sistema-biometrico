...
try {
    ...
    ps = con.prepareStatement("SELECT hora_entrada_oficial, minutos_tolerancia FROM configuracion_horarios LIMIT 1");
    rsConfig = ps.executeQuery();
    LocalTime horaOficial = LocalTime.of(8, 0); // Valor por defecto
    int tolerancia = 0;
    if (rsConfig.next()) {
        horaOficial = rsConfig.getTime("hora_entrada_oficial").toLocalTime();
        tolerancia = rsConfig.getInt("minutos_tolerancia");
    }
...
<%
    ...
    while (rs.next()) {
        ...
        // Cálculo de tardanza si hay hora de entrada
